...
Photosynthesis constraints were converted from area to FW measurements and scaled to a constant flux over an 8 h photoperiod. Constraints were applied to the genome-scale metabolic model of Arnold2014 as done in `ColdAcclimationModels.ipynb` and sampling was done using the CHRR algorithm (`ArabidopsisSampling.m`). Flux distributions were then plotted to a PDF file (`XX`). 
